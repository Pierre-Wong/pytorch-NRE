...
class GRU:
	def __init__(self,is_training,word_embeddings,settings):
		...
		#self.length=tf.constant([self.num_steps for i in range(81)])
		...
		self.length=tf.cast(tf.ones([total_num])*70,tf.int32)
		...
		#inputs_forward=tf.transpose(inputs_forward, perm=[1, 0,2])
		#(outputs, states)=tf.nn.bidirectional_dynamic_rnn(cell_forward, cell_backward,inputs_forward,self.length,self._initial_state_forward,self._initial_state_backward)
		#inputs_forward=tf.transpose(inputs_forward,[1,0,2])
		#outputs,_,_=tf.contrib.rnn.static_bidirectional_rnn(cell_forward,cell_backward,tf.reshape(inputs_forward,[-1]),dtype=tf.float32)
		
		# word-level attention layer
		output_h = tf.add(output_forward,output_backward)
		#output_h=tf.add(outputs[0],outputs[1])
